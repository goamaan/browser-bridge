---
name: browser-bridge
description: Live-first Chrome/CDP browser automation with managed Chromium fallback. Use when an agent needs to inspect pages, attach to an already-open logged-in Chrome tab, resolve and warm tabs by alias, capture snapshots/screenshots/HTML, run browser actions, recover from daemon or browser disconnects, or install and use the browser-bridge CLI skill.
license: MIT
...
Use `browser-bridge` as the primary browser automation tool for live Chrome work.

Prefer installed-binary usage such as `browser-bridge ...`, `bunx @goamaan/browser-bridge ...`, or `npx @goamaan/browser-bridge ...` in consumer-facing workflows.

## Workflow

1. Start or verify the daemon.
2. Attach to the live browser or launch a managed fallback browser.
3. Run `browser-bridge doctor --browser <browserId>` when the session may be stale.
