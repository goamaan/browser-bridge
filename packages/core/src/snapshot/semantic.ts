...
export function snapshotExpression(): string {
  return `(() => {
    const ignoredTags = new Set(['HEAD', 'LINK', 'META', 'NOSCRIPT', 'SCRIPT', 'STYLE', 'TITLE']);
    const interestingRoles = new Set(['button', 'checkbox', 'columnheader', 'combobox', 'heading', 'img', 'link', 'radio', 'row', 'table', 'textbox']);
    const semanticTags = new Set(['A', 'BUTTON', 'IMG', 'INPUT', 'LABEL', 'OPTION', 'SELECT', 'TABLE', 'TD', 'TEXTAREA', 'TH']);
    ...
    const directText = (el) => normalized(Array.from(el.childNodes).filter((node) => node.nodeType === Node.TEXT_NODE).map((node) => node.textContent).join(' '));
    const visible = (el) => {
    ...
    const walk = (root, parentId, framePath) => {
      ...
      const ownText = directText(root).slice(0, 160);
      const isVisible = visible(root);
      const rect = root.getBoundingClientRect();
      const isActionable = interestingRoles.has(role);
      const hasStableHook = root.hasAttribute('data-testid') || root.hasAttribute('aria-label') || Boolean(root.id);
      const isSemantic = semanticTags.has(tagName);
      const isLeafLike = root.children.length === 0 || ownText.length > 0;
      const hasInterestingState = isActionable || hasStableHook || isSemantic || Boolean(role) || Boolean(name) || Boolean(ownText) || (isLeafLike && Boolean(text));
      if (hasInterestingState && tagName !== 'BODY' && tagName !== 'HTML') {
        nodes.push({
          id: nodeId,
          parentId,
          role,
