...
export interface BrowserSessionAdapter {
  ...
  resolvePage(pageRef: string): Promise<PageSummary>;
  doctor(pageRef?: string): Promise<BrowserDoctorResult>;
  ...
  clickPoint(pageRef: string, x: number, y: number): Promise<ActionResult<{ x: number; y: number }>>;
  ...
  insertText(pageRef: string, value: string): Promise<ActionResult<string>>;
  loadAll(pageRef: string, locator: LocatorSpec, intervalMs?: number): Promise<ActionResult<LoadAllDetails>>;
