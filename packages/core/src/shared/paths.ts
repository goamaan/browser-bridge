...
export function browserStateDir(): string {
  const dir = path.join(ensureRuntimeDir(), 'browsers');
  mkdirSync(dir, { recursive: true });
  return dir;
}

export function browserStatePath(browserId: string): string {
  const safeBrowser = browserId.replace(/[^A-Za-z0-9._-]/g, '-');
  return path.join(browserStateDir(), `${safeBrowser}.json`);
}
