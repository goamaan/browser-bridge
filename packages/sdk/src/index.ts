...
export class BrowserBridgeClient {
  ...
  public async detach(browserId = this.requireBrowserId()): Promise<unknown> {
    return await this.daemon.call('browser.detach', { browserId });
  }
  ...
  public async resolvePage(page: string, browserId = this.requireBrowserId()): Promise<PageSummary> {
    return await this.daemon.call<PageSummary>('pages.resolve', { browserId, page });
  }

  public async doctor(browserId = this.requireBrowserId(), page?: string): Promise<BrowserDoctorResult> {
    return await this.daemon.call<BrowserDoctorResult>('browser.doctor', { browserId, page });
  }
...
class PageHandle implements PageLike {
  ...
  public async clickPoint(x: number, y: number): Promise<ActionResult<{ x: number; y: number }>> {
    return await this.daemon.call<ActionResult<{ x: number; y: number }>>('page.clickPoint', {
  ...
  public async insertText(value: string): Promise<ActionResult<string>> {
    return await this.daemon.call<ActionResult<string>>('page.insertText', { browserId: this.browserId, page: this.pageRef, value });
  }

  public async loadAll(locator: LocatorSpec, intervalMs?: number): Promise<ActionResult<LoadAllDetails>> {
    return await this.daemon.call<ActionResult<LoadAllDetails>>('page.loadAll', {
      browserId: this.browserId,
      page: this.pageRef,
      locator,
      intervalMs,
  ...
  public async waitForNetworkIdle(timeoutMs?: number): Promise<ActionResult<string>> {
    return await this.daemon.call<ActionResult<string>>('page.wait', {
      browserId: this.browserId,
      page: this.pageRef,
      options: { networkIdle: true, timeoutMs },
    });
  }
